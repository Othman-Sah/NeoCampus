...
import {
  Loader2, ArrowLeft, ArrowRight, Save, Check, AlertCircle,
  Upload, X, Eye, EyeOff, KeyRound, RefreshCw, ShieldAlert
} from 'lucide-react'
...
export const StudentEditPage: React.FC = () => {
  ...
  // Admin password gate modal state
  const [securityModalOpen, setSecurityModalOpen] = useState(false)
  const [securityAction, setSecurityAction] = useState<'reveal' | 'change-password' | null>(null)
  const [adminPasswordInput, setAdminPasswordInput] = useState('')
  const [securityLoading, setSecurityLoading] = useState(false)
  const [securityError, setSecurityError] = useState<string | null>(null)
  const [revealedPassword, setRevealedPassword] = useState<string | null>(null)

  // New password state (used when securityAction === 'change-password')
  ...
  useEffect(() => {
    if (student) {
      const matchedClass = mockClasses.find(c => c.name && c.name.toLowerCase() === student.classe_nom?.toLowerCase())
      const initialClassId = student.classe_id ? String(student.classe_id) : (matchedClass ? String(matchedClass.id) : '')
      ...
      reset({
        ...
        parent_nom: student.parent_contact?.nom || '',
        parent_relation: student.parent_contact?.relation || 'Father',
        parent_phone: student.parent_contact?.telephone || '',
        parent_email: student.parent_contact?.email || '',
        medical_details: '',
        birth_certificate: student.documents?.birth_certificate !== false,
        previous_transcript: student.documents?.previous_transcript !== false,
        photos: student.documents?.photos !== false,
        classe_id: initialClassId,
        status: (student.status as 'Active' | 'Suspended') || 'Active',
        scolarite_anterieure: student.scolarite_anterieure || '',
      })

      // Set existing avatar as preview if student has one
      if ((student as any).avatar) {
        setPhotoPreview((student as any).avatar)
      }
    }
  }, [student, reset, mockClasses])
  ...
  const handlePhotoChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Upload immediately on selection for edit page
    if (studentId) {
      setUploadingAvatar(true)
      try {
        const newUrl = await uploadAvatar(studentId, file)
  ...
  // Open the admin security gate modal
  const openSecurityModal = (action: 'reveal' | 'change-password') => {
  ...
  const handleSecurityConfirm = async () => {
    ...
    setSecurityError(null)
    try {
      if (securityAction === 'reveal') {
        const pwd = await revealPassword(studentId, adminPasswordInput)
        setRevealedPassword(pwd)
      } else if (securityAction === 'change-password') {
        if (!newPasswordValue || newPasswordValue.trim() === '') {
          setSecurityError('New password cannot be empty.')
          return
        }
        await updatePassword(studentId, adminPasswordInput, newPasswordValue)
        closeSecurityModal()
      }
    } catch (err: any) {
      setSecurityError(err.response?.data?.message || err.message || 'Invalid administrator password.')
    } finally {
  ...
  const handleFormSubmit = async (values: EditFormValues) => {
    ...
    try {
      ...
      const validationDetails = err.response?.data?.errors 
        ...
        : ''
      setErrorMsg(`Failed to update student: ${details}${validationDetails ? ' (' + validationDetails + ')' : ''}`)
    }
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in text-neutral-900 pb-12">
        ...
        <CardContent className="p-8 space-y-6">
          ...
          <div className="border-b border-[#e5e7eb] pb-4 flex justify-between items-center">
            <div>
              ...
              <p className="text-[10px] font-semibold text-neutral-400 uppercase tracking-wide">
                Step {step} of 5 — {step === 5 ? 'Review and save changes' : 'Please modify details'}
              </p>
            </div>
            <div className="text-right">
              <span className="text-[9px] font-black text-neutral-450 uppercase block tracking-wider">Matricule (Read-Only)</span>
              <span className="font-mono text-xs font-bold text-neutral-800">{student?.matricule}</span>
            </div>
          </div>
              ...
              <div className="flex items-center gap-5 border-b border-neutral-100 pb-5">
                ...
                <div className="space-y-1.5 flex-1">
                  ...
                  <p className="text-[9px] font-semibold text-neutral-400 uppercase tracking-wider">
                    JPG, PNG, WebP — max 5 MB{uploadingAvatar ? ' · Uploading...' : ''}
                  </p>
                  <div className="flex gap-2 mt-2">
                    <Button
                      ...
                      className="h-7 px-3 text-[10px] font-bold border-[#e5e7eb] text-neutral-600 hover:text-black rounded-lg cursor-pointer disabled:opacity-50"
                    >
                      {photoPreview ? 'Change Photo' : 'Upload Photo'}
                    </Button>
                    {photoPreview && !uploadingAvatar && (
                      <Button
                        type="button"
                        variant="ghost"
                        onClick={removePhoto}
          ...
          {step === 5 && (
            <div className="space-y-6">
              ...
              {/* Admin-Gated Password Section */}
              <div className="border border-[#e5e7eb] rounded-xl overflow-hidden">
                <div className="bg-[#f9f9f9] px-4 py-3 border-b border-[#e5e7eb] flex items-center gap-2">
                  <ShieldAlert className="h-3.5 w-3.5 text-neutral-500" />
                  <p className="text-[10px] font-black text-neutral-500 uppercase tracking-wider">Portal Password — Admin Only</p>
                </div>
                <div className="p-4 flex items-center justify-between gap-4">
                  <div>
                    <p className="text-xs font-semibold text-neutral-700">Student login password is protected.</p>
                    <p className="text-[9px] text-neutral-400 font-medium mt-0.5">
                      You must verify your admin credentials to reveal or change it.
                    </p>
                  </div>
                  <div className="flex gap-2 shrink-0">
                    <Button
                      ...
                      className="h-8 px-3 text-[10px] font-bold border-[#e5e7eb] text-neutral-600 hover:text-black rounded-lg cursor-pointer gap-1.5"
                    >
                    ...
                    <Button
                      type="button"
                      variant="outline"
                      onClick={() => openSecurityModal('change-password')}
                      className="h-8 px-3 text-[10px] font-bold border-[#e5e7eb] text-neutral-600 hover:text-black rounded-lg cursor-pointer gap-1.5"
                    >
              ...
              <div className="bg-[#f9f9f9] border border-[#e5e7eb] p-4 rounded-xl space-y-2 text-xs">
                <h4 className="font-bold text-neutral-800 uppercase tracking-wider text-[9px] mb-2 text-neutral-400">Review Changes Summary</h4>
                ...
                {photoFile && (
                  <p><span className="text-neutral-500 font-medium">Avatar:</span> <span className="font-bold text-green-600">✓ Photo updated</span></p>
                )}
              </div>
            </div>
          )}
      ...
      <Dialog open={securityModalOpen} onOpenChange={(open) => !open && closeSecurityModal()}>
        <DialogContent className="max-w-sm bg-white rounded-2xl shadow-2xl border border-[#e5e7eb]">
          <DialogHeader>
            <div className="flex items-center gap-2 mb-1">
              ...
              <DialogTitle className="text-sm font-black text-neutral-900">
                {securityAction === 'reveal' ? 'Confirm Admin Credentials' : 'Authorize Password Change'}
              </DialogTitle>
            </div>
            <DialogDescription className="text-[11px] text-neutral-500 font-medium">
              {securityAction === 'reveal'
                ? "Enter your administrator password to reveal this student's portal password."
                : "Enter your administrator password to authorize changing this student's password."}
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-4 py-2">
            {/* Show revealed password if action was 'reveal' and password is known */}
            {revealedPassword ? (
              <div className="p-4 bg-[#f9f9f9] border border-[#e5e7eb] rounded-xl space-y-1">
                <p className="text-[9px] font-black text-neutral-400 uppercase tracking-wider">Current Password</p>
                <p className="text-base font-black text-neutral-900 tracking-widest select-all font-mono">{revealedPassword}</p>
              </div>
            ) : (
              <>
                {/* Admin password input */}
                <div className="space-y-1.5">
                  <Label htmlFor="admin_password_student" className="text-[10px] font-extrabold uppercase text-neutral-500 tracking-wider">
                    Your Admin Password
                  </Label>
                  <Input
                    id="admin_password_student"
                    type="password"
                ...
                {/* New password field for 'change-password' action */}
                {securityAction === 'change-password' && (
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between">
                      <Label htmlFor="new_student_password" className="text-[10px] font-extrabold uppercase text-neutral-500 tracking-wider">
                        New Password (max 8 chars)
                      </Label>
                      <button
                        type="button"
                        onClick={() => setNewPasswordValue(generatePassword())}
                        className="flex items-center gap-1 text-[9px] font-bold text-neutral-400 hover:text-black transition-colors"
                      >
                    ...
                    <div className="relative">
                      <Input
                        id="new_student_password"
                        type={showNewPassword ? 'text' : 'password'}
                        maxLength={8}
                        value={newPasswordValue}
                        onChange={(e) => setNewPasswordValue(e.target.value)}
                        className="bg-white border-[#e5e7eb] text-neutral-900 rounded-lg text-xs h-9 font-mono focus-visible:ring-1 focus-visible:ring-black pr-9"
                      />
                      <button
                        type="button"
                    ...
                    <p className="text-[9px] text-neutral-400 font-medium">
                      {newPasswordValue.length}/8 characters — the student can use this to log in.
                    </p>
                  </div>
                )}
              </>
            ...
            {/* Error message */}
            {securityError && (
              <div className="flex items-center gap-2 p-3 bg-red-50 border border-red-200 rounded-lg text-[10px] text-red-600 font-semibold">
          ...
          <DialogFooter className="flex gap-2 pt-2">
            <Button
              ...
              className="flex-1 h-9 text-xs font-bold border-[#e5e7eb] text-neutral-600 rounded-lg cursor-pointer"
            >
              {revealedPassword ? 'Close' : 'Cancel'}
            </Button>
            {!revealedPassword && (
              <Button
                type="button"
                onClick={handleSecurityConfirm}
                disabled={securityLoading || !adminPasswordInput}
                className="flex-1 h-9 text-xs font-bold bg-black text-white hover:bg-neutral-800 rounded-lg cursor-pointer disabled:opacity-50"
              >
                {securityLoading ? (
                  <Loader2 className="h-3.5 w-3.5 animate-spin" />
                ) : securityAction === 'reveal' ? 'Reveal Password' : 'Update Password'}
              </Button>
            )}
          </DialogFooter>
        </DialogContent>
