...
import { useChildGrades, useChildren } from '@/application/useCases/useParentPortal'
import { ArrowLeft, GraduationCap } from 'lucide-react'

export const ParentChildGradesPage: React.FC = () => {
  const { childId } = useParams<{ childId: string }>()
  const childIntId = parseInt(childId || '0', 10)
  
  const { data: children } = useChildren()
  const { data: grades, isLoading } = useChildGrades(childIntId)

  const child = children?.find(c => c.id === childIntId)

  // Group grades by subject (matiere)
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <h1 className="text-xl font-extrabold tracking-tight">
            Grades Report — {child ? `${child.prenom} ${child.nom}` : 'Child'}
          </h1>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {Object.keys(groupedGrades).length > 0 ? (
                ...
                <div className="flex justify-between items-start pb-3 border-b border-neutral-50">
                  ...
                  {average !== null && (
                    <div className="text-right">
                      ...
                      <span className="text-xs font-extrabold text-teal-600">{average} / 20</span>
                    </div>
                  )}
                </div>
          ...
          <Card className="col-span-full bg-white border border-neutral-100 shadow-sm rounded-2xl p-8 text-center text-neutral-450">
            ...
            <p className="text-xs font-semibold">No grades recorded for this child.</p>
          </Card>
        )}
      </div>
