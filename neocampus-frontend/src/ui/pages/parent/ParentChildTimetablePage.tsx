...
import { useChildTimetable, useChildren } from '@/application/useCases/useParentPortal'
import { TimetableGrid } from '@/ui/components/timetable/TimetableGrid'
import { ArrowLeft, Calendar } from 'lucide-react'

export const ParentChildTimetablePage: React.FC = () => {
  const { childId } = useParams<{ childId: string }>()
  const childIntId = parseInt(childId || '0', 10)

  const { data: children } = useChildren()
  const { data: timetable, isLoading } = useChildTimetable(childIntId)
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="text-xl font-extrabold tracking-tight">
              Timetable — {child ? `${child.prenom} ${child.nom}` : 'Child'}
            </h1>
          </div>
        </div>
        {child && (
      ...
      <Card className="bg-white border border-neutral-100 shadow-sm rounded-3xl p-6 overflow-hidden">
        {isLoading ? (
          ...
          <div className="text-center py-20 text-neutral-405">
            ...
            <p className="text-xs font-semibold">No timetable sessions scheduled for this child's class.</p>
          </div>
        )}
      </Card>
